...
export default function SendCenter({ accountId = 'default' }: { accountId?: string }) {
  const [tab, setTab] = useState<'direct' | 'campaign'>('direct')
  ...
  useEffect(() => {
    ...
    window.zap.getCampaigns().then(rows => setScheduled(rows.filter((row: any) => row.status === 'scheduled').length))
  }, [accountId])
  ...
  return <div>
    ...
    {tab === 'direct' ? <SendMessage accountId={accountId} embedded /> : <MassSend embedded composeOnly />}
  </div>
}
