...
function MediaPanel({ accountId, config, onSaved, colors }: any) {
  const defaults = {
    ...
    voiceEnabled: false, voiceProvider: 'openrouter', voiceUseTextKey: true, voiceReplyMode: 'request', voiceModel: 'google/gemini-3.1-flash-tts-preview', voiceName: 'Kore', voiceSpeed: 1, voiceDailyLimit: 20,
  ...
  async function save() {
    try {
      ...
      let saved = await window.zap.aiSaveConfig(accountId, {
        ...
        voiceEnabled: draft.voiceEnabled, voiceProvider: draft.voiceProvider, voiceUseTextKey: draft.voiceUseTextKey, voiceReplyMode: draft.voiceReplyMode, voiceModel: draft.voiceModel, voiceName: draft.voiceName.trim(), voiceSpeed: Number(draft.voiceSpeed), voiceDailyLimit: Number(draft.voiceDailyLimit),
